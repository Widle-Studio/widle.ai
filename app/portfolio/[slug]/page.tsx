...
// Centralized static data for all Widle projects
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 mt-12 mb-20">
          <AnimateIn direction="up" delay={0.2}>
            <div className="relative aspect-video w-full overflow-hidden rounded-3xl border border-border bg-card shadow-lg p-12">
               {/* Using object-contain due to logo formats from widle.ai */}
              <Image
                src={project.image}
                alt={project.title}
                fill
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!project) {
    return { title: "Project Not Found | Widle" }
  }

  return {
    title: `${project.title} | Portfolio | Widle`,
